...
export default async function shareTradeLinkConfig(request, response) {
  ...
  try {
    ...
    const filteredData = existingData.filter(
      (obj) => !obj.shareWithFriendsChannel
    );
    //save file
    ...
    //save new trade link
    saveToFile(
      { shareWithFriendsChannel: channel },
      `/cs2Tracker/${guildId}.json`
    );
