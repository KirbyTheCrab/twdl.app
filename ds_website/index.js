...
app.get("/auth/discord", authLimiter, async (req, res) => {
...
app.get("/api/status/metrics", isAuthenticated, async (req, res) => {
  const [commandCount, eventCount] = await Promise.all([
    countJsFilesRecursive(join(__dirname, "../ds_bot/commands")),
    countJsFilesRecursive(join(__dirname, "../ds_bot/events")),
  ]);
...
app.get("/api/config/login-url", (req, res) => {
  const redirectUri = encodeURIComponent(`${process.env.BASE_URL}/auth/discord`);
  const loginUrl = `https://discord.com/oauth2/authorize?client_id=${process.env.CLIENT_ID}&response_type=code&redirect_uri=${redirectUri}&scope=identify+guilds+connections+guilds.members.read`;
  const botInviteUrl = `https://discord.com/oauth2/authorize?client_id=${process.env.CLIENT_ID}&scope=bot&permissions=1099511627775`;
  res.json({ loginUrl, botInviteUrl });
});
...
app.listen(port, async () => {
  console.log(`Apps listening at ${process.env.BASE_URL}`);
});
